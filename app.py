...
#课程快捷日志导航
@app.route("/book_date_guide/<int:num>",methods=["GET"])
def book_date_guide(num):
  if request.method == 'GET':
    global date_acc  #获取全局变量
    if num==7:
      date_acc=[(datetime.date.today() + datetime.timedelta(days=-7)),today]
...
#月活折线图
@app.route("/month_line_chart")
def line_month_chart():
